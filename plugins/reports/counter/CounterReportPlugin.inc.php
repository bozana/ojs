...
class CounterReportPlugin extends ReportPlugin
{
                    ...
                    if ($release && $report && $year) {
                        ...
                        if ($release == $this->getCurrentRelease() && isset($available[$report]) && in_array($year, $years)) {
                            ...
                            if ($reporter) {
                                ...
                                $reportItems = $reporter->getReportItems([], [StatisticsHelper::STATISTICS_DIMENSION_MONTH => ['from' => $year . '-01-01', 'to' => $year . '-12-31']]);
                                if ($reportItems) {
    ...
    */
    public function _getYears()
    {
        ...
        $orderBy = [StatisticsHelper::STATISTICS_DIMENSION_YEAR => StatisticsHelper::STATISTICS_ORDER_ASC];
        $results = Services::get('publicationStats')->getMetrics([StatisticsHelper::STATISTICS_DIMENSION_YEAR], $orderBy, $filters)->toArray();
