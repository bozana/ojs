...
class DOIPubIdPlugin extends PubIdPlugin {
	...
	function getLinkActions($pubObject) {
		...
		$userVars['clearParam'] = $this->getClearActionName();
		// Clear object pub id
		...
		if (is_a($pubObject, 'Issue')) {
			...
			$linkActions['excludeIssueObjectsLinkActionURN'] = new LinkAction(
					'excludeIssueObjects',
					new RemoteActionConfirmationModal(
							__('plugins.pubIds.doi.editor.excludeIssueObjectsDoi.confirm'),
							__('common.delete'),
							$request->url(null, null, 'excludeIssueObjects', null, $userVars),
							'modal_delete'
							),
					__('plugins.pubIds.doi.editor.excludeIssueObjectsDoi'),
					'delete'
					);
		}
