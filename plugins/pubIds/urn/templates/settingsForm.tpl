...
<form class="pkp_form" id="urnSettingsForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="pubIds" plugin=$pluginName verb="save"}">
	...
	{fbvFormArea id="urnObjectsFormArea" title="plugins.pubIds.urn.manager.settings.urnObjects"}
		{fbvFormSection list="true" description="plugins.pubIds.urn.manager.settings.explainURNs"}
			...
			{if $enableSubmissionFileURN}
				{assign var="checked" value=true}
			{else}
				{assign var="checked" value=false}
			{/if}
			{fbvElement type="checkbox" label="plugins.pubIds.urn.manager.settings.enableSubmissionFileURN" id="enableSubmissionFileURN" maxlength="40" checked=$checked}
		{/fbvFormSection}
	{/fbvFormArea}
