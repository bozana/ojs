...
<form class="pkp_form" id="urnSettingsForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT op="plugin" category="pubIds" plugin=$pluginName verb="settings" save="true"}">
	...
	{fbvFormArea id="urnPrefixFormArea" class="border" title="plugins.pubIds.urn.manager.settings.urnPrefix"}
		{fbvFormSection title="plugins.pubIds.urn.manager.settings.urnPrefix" for="urnPrefix" description="plugins.pubIds.urn.manager.settings.urnPrefix.description"}
	...
	{fbvFormArea id="urnSuffixFormArea" class="border" title="plugins.pubIds.urn.manager.settings.urnSuffix"}
		...
		{fbvFormSection list="true"}
			...
			{else}
				{assign var="checked" value=false}
			{/if}
			{fbvElement type="radio" id="urnSuffixDefault" name="urnSuffix" value="default" checked=$checked label="plugins.pubIds.urn.manager.settings.urnSuffix.default"}
			{if $doiSuffix eq "publisherId"}
				{assign var="checked" value=true}
			{else}
				{assign var="checked" value=false}
			{/if}
			{fbvElement type="radio" id="urnSuffixPublisherId" name="urnSuffix" value="publisherId" checked=$checked label="plugins.pubIds.urn.manager.settings.urnSuffix.publisherId"}
			...
			{else}
				{assign var="checked" value=false}
			{/if}
			{fbvElement type="radio" id="urnSuffixCustomIdentifier" name="urnSuffix" value="customIdentifier" checked=$checked label="plugins.pubIds.urn.manager.settings.urnSuffix.customIdentifier"}
		{/fbvFormSection}
	{/fbvFormArea}
	...
	{fbvFormArea id="urnNamespaceFormArea" class="border" title="plugins.pubIds.urn.manager.settings.namespace"}
		{fbvFormSection description="plugins.pubIds.urn.manager.settings.namespace.description"}
			{fbvElement type="select" id="urnNamespace" from=$urnNamespaces selected=$urnNamespace translate=false disabled=$readOnly size=$fbvStyles.size.MEDIUM}
		{/fbvFormSection}
	{/fbvFormArea}
	{fbvFormArea id="urnResolverFormArea" class="border" title="plugins.pubIds.urn.manager.settings.urnResolver"}
		{fbvFormSection description="plugins.pubIds.urn.manager.settings.urnResolver.description"}
			{fbvElement type="text" label="plugins.pubIds.urn.manager.settings.urnResolver" id="urnResolver" value=$urnResolver}
		{/fbvFormSection}
	{/fbvFormArea}
