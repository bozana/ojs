...
class DOAJExportPlugin extends PubObjectsExportPlugin
{
    ...
    /*
    public function registerObject($object, $context)
    {
        ...
        // Get the JSON
        $jsonString = $this->exportJSON($object, $filter, $context);
        $jsonDecoded = json_decode($jsonString, true);

        $doajId = $object->getData($this->getIdSettingName());
        if (empty($doajId) && $context->getData(Context::SETTING_DOI_VERSIONING)) {
            $doajId = Repo::publication()->getMinorVersionsSettingValues($object->getData('submissionId'), $object->getData('versionStage'), $object->getData('versionMajor'), $this->getIdSettingName())->first();
        }
        $doajDoi = $object->getData($this->getDoiSettingName());
        $doajUrl = $object->getData($this->getUrlSettingName());
        ...
        if (!empty($doajId) && ($doajDoi != $objectDoi || $doajUrl != $objectUrl)) {
            $jobs = [
                new DOAJDelete($doajId, $object, $context),
                (new DOAJRegister())->delay(now()->addMinutes(10)),
            ];
            Bus::chain($jobs)
