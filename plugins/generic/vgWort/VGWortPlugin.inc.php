...
class VGWortPlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if ($success && $this->getEnabled()) {
			...
			if ($this->authorize()) {
				// Management navigation menu link to VG Wort pages
			...
			HookRegistry::register('Templates::Controllers::Modals::SubmissionMetadata::IssueEntryTabs::AdditionalItems', array($this, 'vgWortEntryTab'));

			// Insert new field into the author metadata form
			/*
			HookRegistry::register('Templates::Controllers::Grid::Users::Author::Form::AuthorForm::AdditionalMetadata', array($this, 'metadataFieldEdit'));
...
$file = 'debug.txt';
$current = file_get_contents($file);
$current .= print_r($formAuthor, true);
file_put_contents($file, $current);
*/
