...
class VGWortPlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if ($success && $this->getEnabled()) {
			...
			// Consider the new field in the author metadata view
			//HookRegistry::register('Templates::Submission::Metadata::Metadata::Authors', array($this, 'metadataFieldView'));
			...
			// Hook for execute in two forms -- consider the new field 
	...
	function displayEditorHomeLinks($hookName, $params) {
		$smarty =& $params[1];
		$output =& $params[2];
		$output .= '<h3>' . TemplateManager::smartyTranslate(array('key'=>'plugins.generic.vgWort.editor.vgWort'), $smarty) . '</h3><ul class="plain"><li>&#187; <a href="' . TemplateManager::smartyUrl(array('op'=>'pixelTags'), $smarty) . '">' . TemplateManager::smartyTranslate(array('key'=>'plugins.generic.vgWort.editor.pixelTags'), $smarty) . '</a></li></ul>';
		return false;
	}
	...
	function metadataExecute($hookName, $params) {
		...
		// set authors data: cardNo
		$formAuthors = $form->getData('authors');
		$articleAuthors =& $article->getAuthors();
		for ($i=0; $i<count($articleAuthors); $i++) {
			$articleAuthors[$i]->setData('cardNo', $formAuthors[$i]['cardNo']);
		}				
		return false;
	}
	...
	function addCheck($hookName, $params) {
		...
		if (get_class($form) == 'AuthorSubmitStep3Form' || get_class($form) == 'MetadataForm' ) {
			...
			//$form->addCheck(new FormValidatorCustom($form, 'authors', 'required', 'plugins.generic.vgWort.cardNoRequired', create_function('$authors', '$allEmpty = true; $allExist = true; foreach ($authors as $author) { if (empty($author[\'cardNo\'])) { $allEmpty = $allEmpty && true; $allExist = $allExist && false; } else { $allEmpty = $allEmpty && false; $allExist = $allExist && true; } } return $allEmpty || $allExist;')));			
		}
		return false;
	}
	...
	function assignPixelTag($hookName, $args) {
		...
		switch ($params['smarty_include_tpl_file']) {
			case 'submission/metadata/metadata.tpl':
				...
				$smarty->display($this->getTemplatePath() . 'assignPixelTag.tpl');
				break;
		}
		return false;	
	}
			...
			if($galley->isHtmlGalley() || $galley->isPdfGalley()) {
				...
				if (isset($publishedArticle)) {
					...
					if ($issue->getPublished()) {
						...
						$pixelTagDao =& DAORegistry::getDAO('PixelTagDAO');
						$pixelTag =& $pixelTagDao->getPixelTagByArticleId($journal->getId(), $publishedArticle->getArticleId());
