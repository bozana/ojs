...
class PR
{
    ...
    public function getReportHeader(): array
    {
        $reportHeader = [
            ...
            // use the context primary locale name, to be always consistent
            'Created_By' => $this->platformName,
            'Customer_ID' => (string) $this->customerId,
            'Report_ID' => $this->getID(),
            'Report_ID' => $this->getRelease(),
        ...
        $reportHeader['Report_Filters'] = $this->filters;
        if (!empty($this->attributes)) $reportHeader['Report_Attributes'] = $this->attributes;
        if (!empty($this->exceptions)) $reportHeader['Exceptions'] = $this->exceptions;
