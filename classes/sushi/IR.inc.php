...
class IR
{
    ...
    public function getReportItems(): array
    {
        ...
        foreach ($results as $result) {
            ...
            if (in_array('YOP', $this->attributesToShow)) {
                $resultsGroupedBySubmission[$result->submission_id][$result->YOP][] = $result;
            } else {
        ...
        foreach ($resultsGroupedBySubmission as $submissionId => $submissionResults) {
            if (in_array('YOP', $this->attributesToShow)) {
                $yearOfPublication = array_key_first($submissionResults);
                $results = $submissionResults[$yearOfPublication];
            } else {
            ...
            $item['Item_ID'][] = [
                'Type' => 'Proprietary',
                'Value' => $context->getPath() . ':' . $submissionId,
            ];
            ...
            if ($this->includeParentDetails == 'True') {
                ...
                $parentItem['Item_ID'] = [
                    ['Type' => 'Proprietary', 'Value' => $context->getPath() . ':' . $context->getId()]
                ];
