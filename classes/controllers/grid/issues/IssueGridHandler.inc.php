...
class IssueGridHandler extends GridHandler {
	...
	function IssueGridHandler() {
		...
		$this->addRoleAssignment(
			...
			array(
				...
				'pubIds', 'updatePubIds', 'clearPubId', 'issueObjectsPubIdActions',
			)
		);
	}
	...
	function updatePubIds($args, $request) {
		...
		if ($form->validate($request)) {
			$form->execute($request, $actionType);
			return DAO::getDataChangedEvent($issue->getId());
		} else {
			return new JSONMessage(true, $form->fetch($request));
		}
	 ...
	 * @return JSONMessage JSON object
	 */
	function issueObjectsPubIdActions($args, $request) {
		$issue = $this->getAuthorizedContextObject(ASSOC_TYPE_ISSUE);
		import('controllers.tab.pubIds.form.PublicIdentifiersForm');
		$form = new PublicIdentifiersForm($issue);
		$form->issueObjectsPubIdsActions($request->getUserVar('pubIdPlugIn'), $request->getUserVar('actionType'));
		return new JSONMessage(true);
	}
