...
class StatsListQueryBuilder extends BaseQueryBuilder {
	 ...
	 * Assumes that lookingFor and searchPhrase are already set
	 *
	 * @return \OJS\Services\QueryBuilders\StatsListQueryBuilder
	 */
	public function filterBySearchPhrase() {
		switch ($this->lookingFor) {
			case 'orderedSubmissionsStats':
			case 'totalSubmissionsStats':
			case 'submissionStats':
				$submissionService = \ServicesContainer::instance()->get('submission');
				$submissions = $submissionService->getSubmissions($this->contextId, array('searchPhrase' => $this->searchPhrase));
	 ...
	 */
	public function get() {
		// search phrase is currently for submission properties, so
		// first we will filter the submissions and then use the filtered
		// submission IDs to get the stats
		if ($this->searchPhrase != null) {
			$this->filterBySearchPhrase($this->searchPhrase);
		}
