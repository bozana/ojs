...
class PubIdExportSubmissionsListGridHandler extends GridHandler {
	...
	function renderFilter($request, $filterData = array()) {
		...
		$status = $this->_plugin->getStatusMapping();
		// TO-THINK: This actualy belongs to the parent PubIdExportPlugin class:
		$status[0] = __('plugins.importexport.common.status.any');
		$status[1] = __('plugins.importexport.common.status.notDeposited');
		ksort($status);
		...
		$filterData = array(
				...
				'status' => $status,
				'gridId' => $this->getId(),
				'pubIdType' => $this->_plugin->getPubIdType()
		);
	...
	function getFilterSelectionData($request) {
		...
		$statusId = (int) $request->getUserVar('statusId');
		return array(
				'search' => $search,
				'column' => $column,
				'issueId' => $issueId,
	...
	protected function loadData($request, $filter) {
		...
		list($search, $column, $issueId, $statusId, $pubIdType) = $this->getFilterValues($filter);
		$title = $author = null;
		if ($column == 'title') {
			$title = $search;
		...
		if ($statusId) {
			$pubIdStatusSettingName = $this->_plugin->getPluginId().'::status';
		}
	...
	protected function getFilterValues($filter) {
		...
		if (isset($filter['statusId']) && $filter['statusId'] > 1) { //0 = any, 1 = not deposited
			$statusId = $filter['statusId'];
		} else {
			$statusId = null;
		}

		if (isset($filter['pubIdType']) && $filter['pubIdType']) {
			$pubIdType = $filter['pubIdType'];
		} else {
			$pubIdType = $this->_plugin->getPubIdType();
		}

		return array($search, $column, $issueId, $statusId, $pubIdType);
	}
